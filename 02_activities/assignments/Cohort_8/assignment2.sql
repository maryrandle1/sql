...
--Approach using DENSE_RANK (), PART 1
SELECT
	customer_id,
	market_date,
...
--Approach using DENSE_RANK (), PART 2
SELECT
	customer_id,
	market_date,
	visit_number
...
SELECT
	...
	market_date,
	quantity,
	count (*) OVER (PARTITION BY customer_id, product_id) AS times_customer_bought_product
FROM customer_purchases;
...
SELECT
    vendor_name,
    product_name,
    SUM(5 * original_price) AS total_dollars_made
FROM all_sales
